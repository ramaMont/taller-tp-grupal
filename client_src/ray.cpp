...
bool Ray::has_element(const Coordinates &map_coordinates){
	...
	if(map.obtenerPosicionableEn(whole_coordinates)!=nullptr){
		return true;
	}else{
		return false;
	}
...
float Ray::get_distance_to_player_plane(const Coordinates &object_coordinates,
...
float Ray::search_ray_distance(Coordinates ray_position){
	// Qué triangulo use despues me afecta en calcular 
	...
	if(has_element(coordinates_map)){
		float distance_player_plane = get_distance_to_player_plane(
