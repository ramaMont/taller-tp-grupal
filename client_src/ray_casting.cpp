...
Raycasting::Raycasting(Player &a_player,Mapa &a_map,const Window &window):
        ...
        renderer(window.getRenderer()), background(window){
    h = 480;
    n_rays = 160;
    init_textures();
}
...
void Raycasting::draw(Intersected_object intersected_object,float pos_x){
	float distance_player_plane = 
        intersected_object.get_distance_player_plane();
    ...
    // No sé como llamar ésto, es simplemente un calculo q hago 
    // acá para no hacer muchas veces despues
	float x_lenght_ray = 640/(2*n_rays);
